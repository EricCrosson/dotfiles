...
* Paired insertion
#+BEGIN_SRC emacs-lisp :tangle yes
  (defmacro esc/displaced-yank (char back)
    "Charert character CHAR ARG times and then move backward BACK
  characters.

  Note that ARG may only be supplied by an interactive prefix if the
  defun wrapping this macro names its (interactive 'p') argument ARG."
    `(dotimes (i ARG) (insert ,char) (backward-char ,back)))
  ...
  (defmacro esc/define-displaced-yank (funcname data)
    (let ((funsymbol (intern (format "esc/yank-displaced-%s" funcname)))
      ...
      `(defun ,funsymbol (&optional ARG)
         "See displaced-yank.el for more documentation."
         (interactive "p")
         (esc/displaced-yank ,char ,back))))
