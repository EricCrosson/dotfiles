...
#+BEGIN_SRC emacs-lisp :tangle yes
  ...
  region in `org-mode'."
    (let ((funcname (intern (format "esc/org-%s-headers-in-region" funcname)))
          (docstring (format "%s `org-mode' headers arg times in currently selected region."
                             funcname)))
    `(defun ,funcname (&optional arg)
       ,docstring
       (interactive "p")
  ...
  (mapcar (lambda (data)
            (let ((funcname (car data))
                  (callback (cadr data)))
              (esc/define-org-header-indentation funcname callback)))
          '((indent (insert "*"))
  ...
  (defmacro esc/define-scroll-slight(funcname)
    ...
    (let ((funcname (format "esc/scroll-%s-slight" funcname))
          (docstring (format "Scroll %s the page ARG times without moving point. The number of lines to scroll is determined by the variable `esc-line-shift-amount'.

  This command can be prefixed." funcname))
          (command (intern (format "scroll-%s" funcname))))
      `(defun ,funcname(i arg)
         ,docstring
         (interactive "p")
         (dotimes (i arg)
           (,command esc-line-shift-amount)))))

  (mapcar (lambda (data) `(esc/define-scroll-slight ,data))
          '(up down))

  (defmacro esc/define-zoom(funcname command)
    "Define functions to zoom into and out of a buffer."
    (let ((funcname (format "esc/zoom-%s" funcname))
          (docstring (format "Zoom %s on the font in all buffers by `esc-zoom-amount' points.
  ...
  This command can be prefixed." funcname)))
      `(defun ,funcname(arg)
         ...
         (interactive "p")
         (dotimes (i arg)
           (,command esc-zoom-amount)))))

  (mapcar (lambda (data)
            (let ((funcname (car data))
                  (command (cadr data)))
              `(esc/define-zoom ,funcname ,command)))
          '((in +)
            (out -)))
#+END_SRC
