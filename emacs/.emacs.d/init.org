...
#+BEGIN_SRC emacs-lisp :tangle yes
  ...
  (defmacro esc/org-define-header-indentation-defuns(funsymbol callback)
    "Define functions to manipulate the header level of selected
  region in `org-mode'."
    (let ((funsymbol (intern (format "esc/org-%s-headers-in-region" funsymbol)))
  ...
  (mapcar (lambda (data)
            (let ((funsymbol (car data))
                  (callback (cadr data)))
              (esc/org-define-header-indentation-defuns funsymbol callback)))
          '((indent (insert "*"))
