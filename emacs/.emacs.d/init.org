  ...
  (use-package esc-mode
    ...
    (after 'hydra
      ...
      (defhydra hydra-scroll-buffer (esc-mode-map "M-m")
        ...
        ("h" backward-char "bk")
        ("l" forward-char "fw")
        ("f" forward-page "fw.pg")
        ("b" backward-page "bk.pg")
        ("n" next-line "next")
...
*** Save desktop config
#+BEGIN_SRC emacs-lisp :comments link :tangle yes
  (defadvice desktop-save-in-desktop-dir (around ensure-desktop-dir-exists activate)
...
** VERIFY advise save-desktop
to create the dir it's going to save in
** TODO ido-switch-buffers hide from list
buffers for asynchronous shell commands
* TODO work branch
