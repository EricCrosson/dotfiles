...
#+BEGIN_SRC emacs-lisp
  (use-package fic-mode
    ...
    (push "CONTRIBUTE" fic-highlighted-words)
...
#+BEGIN_SRC emacs-lisp
  (use-package rust-mode
    :ensure t
    :pin melpa
    :after (lsp-mode lsp-ui tree-sitter)
...
#+begin_src emacs-lisp
  (use-package js
    :after (evil evil-leader lsp-mode lsp-ui tree-sitter add-node-modules-path)
    ...
    (if buffer-file-name
        (if (string-equal "package.json" (file-name-nondirectory buffer-file-name))
          ...
          (let ((manifest (format "%s/package.json"
            ...
            (when manifest
              (find-file manifest))))
      (message "js-find-package-json error: not currently visiting a file")))
#+end_src

**** find-tsconfig-json
TODO pull this into its own package
#+begin_src emacs-lisp
  (defun ts-find-tsconfig-json ()
    "Switch to the nearest tsconfig.json file above current file.
     If already viewing a tsconfig.json file, run `bury-buffer'."
    (interactive)
    (if buffer-file-name
        (if (string-equal "tsconfig.json" (file-name-nondirectory buffer-file-name))
            (bury-buffer)
          (let ((manifest (format "%s/tsconfig.json"
                                  (locate-dominating-file "." "tsconfig.json"))))
            (when manifest
              (find-file manifest))))
      (message "ts-find-tsconfig-json error: not currently visiting a file")))
#+end_src

**** run-prettier-fix
#+begin_src emacs-lisp
...
#+BEGIN_SRC emacs-lisp
  (use-package prettier
    :ensure t
    :config
    (defun esc/prettier-on-region-or-buffer ()
      (interactive)
      (if (use-region-p)
...
#+BEGIN_SRC emacs-lisp
  (use-package json-mode
    :ensure t
    :pin melpa-stable
    :after (evil-leader tree-sitter add-node-modules-path)
    :hook ((json-mode . tree-sitter-hl-mode)
           (json-mode . add-node-modules-path))
    :config
      (evil-leader/set-key-for-mode 'json-mode
        "fp" 'js-find-package-json
        "ft" 'ts-find-tsconfig-json
        "ff" 'esc/prettier-on-region-or-buffer
...
#+BEGIN_SRC emacs-lisp
  (use-package typescript-mode
    ...
    :pin melpa
    :mode ("\\.tsx\\'")
    :after (evil evil-leader lsp-mode lsp-ui prettier tree-sitter add-node-modules-path)
...
#+BEGIN_SRC emacs-lisp
  (use-package go-mode
    :ensure t
    :pin melpa-stable
    :after (evil lsp-mode tree-sitter)
...
Fall back to evil-args for the outer-parameter text object: https://github.com/meain/evil-textobj-tree-sitter/issues/33
...
#+BEGIN_SRC emacs-lisp
  (use-package counsel-projectile
    ...
    (setq counsel-rg-base-command '("rg" "--max-columns" "240" "--with-filename" "--no-heading" "--line-number" "--color" "never" "-j" "1" "%s"))
...
#+BEGIN_SRC emacs-lisp
  (use-package flycheck
    ...
    ;; must be evaluated after modes are configured
