...
func TestConfigureAnthropicDefaults(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "no model specified - injects opusplan default",
			args:     ParsedArgs{filteredArgs: []string{"--chat"}},
			wantArgs: []string{"--chat", "--model", "opusplan"},
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for i, arg := range finalArgs {
				...
				if arg == "--model" && i+1 < len(finalArgs) && finalArgs[i+1] == "opusplan" && !parsed.hasModel {
